...
export function HowToGet({
  ...
  const midY = ((spot.map.y + ME.y) / 2) * mapH;
  const offset = Math.min(Math.max(midY - HEIGHT / 2, 0), Math.max(mapH - HEIGHT, 0));
  ...
  return (
    <View style={styles.wrap}>
      ...
      <Pressable
        ...
        style={({ pressed }) => [styles.map, pressed && { opacity: 0.92 }]}>
        {width > 0 ? (
        ...
        <Handwritten rotate={-7} size={17} style={styles.note}>
...
const styles = StyleSheet.create({
  ...
  note: { position: 'absolute', left: 14, top: 12 },
